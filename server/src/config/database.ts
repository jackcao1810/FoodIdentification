...
export async function initDatabase(): Promise<void> {
  ...
  const result = database.exec('SELECT COUNT(*) as count FROM dishes');
  ...
  if (count === 0) {
    ...
    for (const dish of dishes) {
      database.run(
        `INSERT INTO dishes (name, category, description, calories_per_100g, protein, carbohydrates, fat, fiber, sodium) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
        dish
      );
    }
