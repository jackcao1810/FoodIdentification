...
本系统采用前后端分离的微服务架构设计，前端基于React构建单页应用，后端采用Node.js + Express提供RESTful API服务，AI推理功能通过TensorFlow.js在前端和后端协同完成。
...
│  │  │              React SPA 前端应用                          │   │   │
│  │  │  ┌─────────┐ ┌─────────┐ ┌─────────┐ ┌───────────────┐  │   │   │
│  │  │  │图片处理 │ │状态管理 │ │路由导航 │ │AI模型推理     │  │   │   │
│  │  │  │组件     │ │(Redux)  │ │(React   │ │(TF.js本地)    │  │   │   │
│  │  │  │         │ │         │ │ Router) │ │               │  │   │   │
│  │  │  └─────────┘ └─────────┘ └─────────┘ └───────────────┘  │   │   │
│  │  └─────────────────────────────────────────────────────────┘   │   │
│  └─────────────────────────────────────────────────────────────────┘   │
...
│  ┌─────────────────────────────────────────────────────────────────┐   │
│  │                       HTTPS/WSS 安全通信层                        │   │
│  │           (HTTP + TLS 1.3) / (WebSocket for实时通信)            │   │
│  └─────────────────────────────────────────────────────────────────┘   │
│                                    │                                     │
└────────────────────────────────────┼────────────────────────────────────┘
                                     │
...
│  │         ▼              ▼              ▼              ▼          │   │
│  │  ┌───────────┐ ┌───────────┐ ┌───────────┐ ┌───────────────┐   │   │
│  │  │认证服务   │ │菜品服务   │ │热量服务   │ │营养建议服务   │   │   │
│  │  │/auth     │ │/api/dishes│ │/api/      │ │/api/          │   │   │
...
前端技术栈选择React + TypeScript是为了获得良好的开发体验和类型安全保障。Ant Design作为UI组件库提供了丰富的企业级组件，能够快速构建美观且功能完善的界面。React Camera Pro专门用于处理移动端的摄像头拍照功能，支持相机预览、拍照、录制等操作。Redux Toolkit作为状态管理方案，能够集中管理应用的全局状态，包括用户信息、识别历史、营养数据等。TensorFlow.js在前端运行可以实现菜品识别的本地化推理，保护用户隐私的同时减少服务器压力。
...
│                         前端数据流                                      │
│                                                                        │
│   ImageUploader ──▶ ImagePreprocessor ──▶ TensorFlow.js               │
│        │                     │                      │                  │
│        ▼                     ▼                      ▼                  │
│   本地预览          Base64/Blob格式         识别结果                    │
│                          │                      │                      │
│                          ▼                      ▼                      │
│                   ┌──────────────┐     RecognitionResult               │
│                   │ Redux Store  │            │                        │
